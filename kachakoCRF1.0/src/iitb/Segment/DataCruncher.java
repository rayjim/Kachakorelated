...
class DCTrainRecord implements TrainRecord, Segmentation {
    ...
    DCTrainRecord(int[] ts, String[][] toks) {
        ...
        }//calculate the length for the token
        ...
        for (int i = 0; i < ls.length; i++) {
            for (int p = 0; p < _tokens[i].length; p++) {
                snum[pos] = i;
                labelsPerToken[pos++] = ls[i];  //results: for each token there will be an assigned label by ray
            }
        }
    }
    public int[] labels() {
	...
	public static TrainData readTagged(int numLabels, String tfile,
		...
		try {
			...
			while ((rawLine = rin.readLine()) != null) {
				...
				td.add(new DCTrainRecord(at, c));//at is the label(converted by -1), c is the token
				//ad training sample to td
			}
